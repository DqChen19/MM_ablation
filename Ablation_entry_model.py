"""
This code models the entry of iron micrometeorites into an atmosphere. 
This code was originally developed by with this code email Owen Lehmer info@lehmer.us.  - 1/14/19
This code is further updated to implement different atmosphere situation and kinetics. contact Danqiu Chen for info.
"""
...
class initialVelocityDistribution(stats.rv_continuous):
    ...
    def _pdf(self, x):
        prob = 0
        a,b = 11.2,20.0 # upper bound can be changed
        if a <= x <= b:
            prob = 1 / (b - a)
        else:
            prob = 0
...
class initialMassDistribution(stats.rv_continuous):
    ...
    def __init__(self):
        ...
        """
        super().__init__(a=2.346E-8, b=0.02932*10) #used to be 2.346e-10


    def _pdf(self, x):
        prob = 0
        if 2.346E-8 < x < 0.02932*10: #used to be 2.3464-10
...
generateRandomSampleData(output_dir="PureN2_upper_20/co2_0"%(
